...
$(function() {
	$.fn.smohanfacebox = function(options) {
	    ...
	    for(i=0;i<60;i++){  //通过循环创建60个表情，可扩展
		 faceimg+='<li><a href="javascript:void(0)"><img src="http://c.com/public/images/face/'+(i+1)+'.png" width="20" face="http://c.com/public/images/face/'+(i+1)+'.png"/></a></li>';
		 };
		...
		$facepic.off().click(function(){
			 ...
			 $("#"+options.textid).off().insertContent("<img src='"+$(this).attr("face")+"'/>");
			 $btn.removeClass('in');
			});			
		//关闭表情层
 ...
 $.fn.extend({
	  replaceface : function(faces){
		  for(i=0;i<60;i++){
			  faces=faces.replace('<emt>'+ (i+1) +'</emt>','<img src="http://c.com/public/images/face/'+(i+1)+'.gif">');
			  }
		   $(this).html(faces);
		   }
	  });
