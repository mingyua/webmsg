    ...
    public function backup($table, $start)
    {
        ...
        if(is_array($table)){
        	foreach($table as $k=>$v){
	            $result = $db->query("SHOW CREATE TABLE `{$v['name']}`");
	            $sql = "\n";
	            $sql .= "-- -----------------------------\n";
	            $sql .= "-- Table structure for `{$v['name']}`\n";
	            $sql .= "-- -----------------------------\n";
                ...
                $sql = "-- -----------------------------\n";
                $sql .= "-- Records of `{$v['name']}`\n";
                $sql .= "-- -----------------------------\n";
                $this->write($sql);
	            //备份数据记录
	            $result = $db->query("SELECT * FROM `{$v['name']}`");
	            foreach ($result as $row) {
	                $row = array_map('addslashes', $row);
	                $sql = "INSERT INTO `{$v['name']}` VALUES ('" . str_replace(array("\r", "\n"), array('\\r', '\\n'), implode("', '", $row)) . "');\n";
	                if (false === $this->write($sql)) {
	                    return false;
	                }
	            }	            	
